...
class Cutting {
    ...
    private void setCuttingBounds () {
        ...
        int indexMinDist = indexMinMaxPointsToPlane(microtome.getCurrentKnifeCentre(),
...
//            Check unit normal points from the current view point to the plane
            ...
            moveToPosition(microtome.getBdvStackSource(), currentViewCentreDouble, 0);
        }
//        System.out.println(Arrays.toString(knifeNormalDouble));
        // TODO - broader check, and not rotated properly
        if (!normalsParallel) {
            levelCurrentViewNormalandHorizontal(microtome.getBdvStackSource(), knifeNormalDouble, edgeVectorDouble);
        }
